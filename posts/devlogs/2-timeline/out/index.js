(() => {
  ...
  // src/index.jsx
  var starfieldVertexShaderSource = `#version 300 es
...
`;
  var starfieldFragmentShaderSource = `#version 300 es
...
void main() {
  ...
  function runTimelineExample() {
    ...
    const starfieldVertexShader = createShader(gl, gl.VERTEX_SHADER, starfieldVertexShaderSource);
    const starfieldFragmentShader = createShader(gl, gl.FRAGMENT_SHADER, starfieldFragmentShaderSource);
    const starfieldProgram = createProgram(gl, starfieldVertexShader, starfieldFragmentShader);
    ...
    const textures = {};
    {
      ...
      gl.vertexAttribPointer(
        starfieldPositionAttributeLocation,
        size,
        type,
        normalize,
        stride,
    ...
    function renderSprite(sprite) {
      ...
      const transformM = multiplyMatrix(
        ...
        multiplyMatrix(
          rotationM,
          multiplyMatrix(scaleM, moveOriginMatrix)
        )
      );
    ...
    function renderStarfield(t) {
      ...
      gl.uniform1f(starfieldTimeUniformLocation, t / 2e3);
      gl.drawArrays(gl.TRIANGLES, 0, 6);
    }
    function update() {
      ...
      ship.health = lerp(ship.health, shipTarget.health, 0.1);
