import { ROUTINE, capitalize, dateKey, exerciseById, formatWeight, isTrainingDay, nearestLoadable, plateLoad, recommendation, scheduledDay, setsFor, targetFor } from './routine.js';

const app = document.querySelector('#app');
const sheetRoot = document.querySelector('#sheet-root');
const DB_NAME = 'plates-workouts';
const DB_VERSION = 1;
let workouts = [];
let view = 'today';
let month = new Date();
...
const prettyDate = (key, short = false) => localDate(key).toLocaleDateString(undefined, short ? { month:'short', day:'numeric' } : { weekday:'long', month:'long', day:'numeric' });
const id = () => crypto.randomUUID ? crypto.randomUUID() : `${Date.now()}-${Math.random()}`;

function openDatabase() {
  return new Promise((resolve, reject) => {
    const request = indexedDB.open(DB_NAME, DB_VERSION);
    request.onupgradeneeded = () => { const db = request.result; if (!db.objectStoreNames.contains('workouts')) db.createObjectStore('workouts', { keyPath:'id' }); };
    request.onsuccess = () => resolve(request.result); request.onerror = () => reject(request.error);
  });
}
async function loadWorkouts() { const db = await openDatabase(); workouts = await new Promise((resolve, reject) => { const req = db.transaction('workouts').objectStore('workouts').getAll(); req.onsuccess = () => resolve(req.result); req.onerror = () => reject(req.error); }); workouts.sort((a,b) => a.date.localeCompare(b.date)); }
async function saveWorkout(workout) { const db = await openDatabase(); await new Promise((resolve, reject) => { const req = db.transaction('workouts', 'readwrite').objectStore('workouts').put(workout); req.onsuccess = resolve; req.onerror = () => reject(req.error); }); const index = workouts.findIndex(item => item.id === workout.id); if (index < 0) workouts.push(workout); else workouts[index] = workout; workouts.sort((a,b) => a.date.localeCompare(b.date)); }
async function replaceAll(next) { const db = await openDatabase(); await new Promise((resolve, reject) => { const tx = db.transaction('workouts', 'readwrite'); tx.objectStore('workouts').clear(); next.forEach(item => tx.objectStore('workouts').put(item)); tx.oncomplete = resolve; tx.onerror = () => reject(tx.error); }); workouts = next.sort((a,b) => a.date.localeCompare(b.date)); }

function dayWorkout(date) { return workouts.find(workout => workout.date === date); }
...
function mappingSheet(payload) { importPayload=payload; const all=payload.workouts; const newest=all.at(-1); const suggested=dateKey(new Date()); showSheet(`<h2>Assign the old workout dates</h2><p>Your iPhone note has order but no dates. Choose the date for its newest workout; nothing is saved until you review the mapping.</p><label for="anchor">Newest ${capitalize(newest.day)} workout</label><input id="anchor" type="date" value="${suggested}"><div class="sheet-actions"><button class="primary" data-action="propose-dates">Review dates</button><button class="secondary" data-action="close-sheet">Cancel</button></div>`); }
function buildMapping(anchor) { let date=anchor; const result=[]; for (let index=importPayload.workouts.length-1; index>=0;index--) { result[index]=date; date=previousTrainingDate(date); } return result; }
function reviewMapping() { const anchor=document.querySelector('#anchor').value; if (!anchor || !isTrainingDay(anchor)) return toast('Choose a Monday–Friday anchor date'); const dates=buildMapping(anchor); showSheet(`<h2>Review date mapping</h2><p>Edit any date before importing. The original sequence and notes are preserved.</p><div class="mapping">${importPayload.workouts.map((workout,index)=>`<label class="map-row"><span>#${workout.sequence || index+1} · ${capitalize(workout.day)}</span><input type="date" data-map="${index}" value="${dates[index]}"></label>`).join('')}</div><div class="sheet-actions"><button class="primary" data-action="save-mapped" data-mode="merge">Import & merge</button><button class="secondary danger" data-action="save-mapped" data-mode="replace">Replace all</button><button class="secondary" data-action="close-sheet">Cancel</button></div>`); }
function normalizeImport(payload, dates) { return payload.workouts.map((workout,index) => ({...workout, id:workout.id || id(), date:dates?.[index] || workout.date, importedAt:new Date().toISOString()})); }
async function importWithDates(mode) { const dates=[...document.querySelectorAll('[data-map]')].map(input=>input.value); if (dates.some(date=>!date)) return toast('Every workout needs a date'); const incoming=normalizeImport(importPayload,dates); if (mode === 'replace') { if (!confirm('Replace every workout currently stored on this phone?')) return; await replaceAll(incoming); } else { const byDate=new Map(workouts.map(workout=>[workout.date,workout])); incoming.forEach(workout=>byDate.set(workout.date,workout)); await replaceAll([...byDate.values()]); } sheetRoot.innerHTML=''; view='calendar'; month=localDate(dates.at(-1)); toast(`${incoming.length} workouts imported`); render(); }
async function receiveImport(payload) { if (!payload?.workouts || !Array.isArray(payload.workouts)) return toast('That JSON does not contain workouts'); if (payload.workouts.some(workout=>!workout.date)) return mappingSheet(payload); importPayload=payload; showSheet(`<h2>Ready to import</h2><p>${payload.workouts.length} dated workouts found. Merge keeps unrelated dates; replacement erases the current local history.</p><div class="sheet-actions"><button class="primary" data-action="import-dated" data-mode="merge">Import & merge</button><button class="secondary danger" data-action="import-dated" data-mode="replace">Replace all</button><button class="secondary" data-action="close-sheet">Cancel</button></div>`); }
async function importDated(mode) { const incoming=normalizeImport(importPayload); if (mode === 'replace') { if (!confirm('Replace every workout currently stored on this phone?')) return; await replaceAll(incoming); } else { const byDate=new Map(workouts.map(workout=>[workout.date,workout])); incoming.forEach(workout=>byDate.set(workout.date,workout)); await replaceAll([...byDate.values()]); } sheetRoot.innerHTML=''; toast(`${incoming.length} workouts imported`); render(); }
